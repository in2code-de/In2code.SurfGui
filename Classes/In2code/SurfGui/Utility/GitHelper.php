...
class GitHelper {
	 ...
	 * @return array
	 */
	protected function getGitEntitiesFromRepositoryUrl($type, $repositoryUrl, $filterRegex, $additionalArguments = array()) {
		$arguments = array(
			'command' => 'ls-remote',
			'options' => array(
				'--' . $type
		...
		$result = $this->executeCommandFromArguments($arguments, function ($branch) use ($filterRegex) {
	...
	protected function executeCommandWithShell($command) {
		...
		if (($returnInteger = $shell->tearDown()) > 0) {
			throw new \Exception('The command "' . htmlspecialchars($command) . '" did not execute successfully', $returnInteger);
		}
		return $output;
	}
}
